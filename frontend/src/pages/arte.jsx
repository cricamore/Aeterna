...
import Experience from "../three/ModuloArte/Experience";
import Evaluacion from "../three/ModuloArte/evaluacion";
      ...
      { isExperienceLoaded ? (<AppBar position="fixed" color="primary" style={{ backgroundColor: '#13192F', width: '200px', height: '100vh', left: 0 }}>
        <Toolbar sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <Box display="flex" alignItems="center" flexDirection="column" mt={4}>
            ...
            {/* Go Home */}
            <Box mt={1}>
                <Link href='/main' onClick={() => { setIsLoading(true) }} passHref>
            ...
            <Box component={Paper} elevation={3} mt={4} p={2} sx={{ width: '150px', height:'10px',backgroundColor: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px solid gray', borderRadius:'20px', cursor:'pointer'}}>
              <Typography variant="subtitle2" color="inherit" sx={{fontSize:'18px',fontWeight:'bold', color:'#D4AF37', display:'flex', alignItems:'center', justifyContent:'center',width:'100%'}} onClick={()=>handleCameraMove(4)}>ARTE</Typography>
            </Box>
            {/* Lecciones*/}
